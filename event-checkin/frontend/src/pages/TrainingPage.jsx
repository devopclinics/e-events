    ...
    {tab === 'learn' ? <div className="training-layout">
      <aside className="training-catalog">
        {data.course.modules.map(module => <section key={module.key}><h2>{module.title}</h2>{module.lessons.map((lesson, i) => {
          ...
          return <button key={lesson.key} className={`${selected?.key === lesson.key ? 'selected' : ''} ${done ? 'done' : ''}`} onClick={() => { setSelected(lesson); setAnswers([]); setEvidence(''); setMessage('') }}>
      ...
      <article className="training-lesson">
        ...
        {data.progress[selected.key]?.status === 'completed' && selectedIndex < lessons.length - 1 && <button className="next" onClick={() => { setSelected(lessons[selectedIndex + 1]); setAnswers([]); setEvidence(''); window.scrollTo({ top: 0, behavior: 'smooth' }) }}>Next lesson →</button>}
      </article>
