...
export default function TicketingOperationsPanel({eventId, sales, checkoutFields=[], onSaveCheckoutFields, deliverySettings={}, onSaveDeliverySettings, canBootstrap=false, onRefresh, onError}) {
  ...
  return <section className="rr-panel tk-section" aria-labelledby="ticket-actions-title">
    ...
    <div className="tk-report-email"><label><b>Checkout form builder</b><span>Collect additional information with every order. Give fields the same step name to group them into a page of a multi-step checkout, navigated with Next/Back — leave step blank to keep a field on the first page.</span></label><button type="button" onClick={()=>setFields([...fields,{id:`field_${Date.now()}`,label:'',type:'text',required:false,options:[],step:''}])}>Add field</button><button type="button" disabled={!!busy} onClick={()=>act('fields',async()=>{await onSaveCheckoutFields(fields);return 'Checkout form published.'})}>Publish form</button></div>
    {fields.map((field,index)=><div className="tk-report-email" key={field.id}><input aria-label="Field label" placeholder="Question or field label" value={field.label} onChange={e=>{const next=[...fields];next[index]={...field,label:e.target.value};setFields(next)}}/><select value={field.type} onChange={e=>{const next=[...fields];next[index]={...field,type:e.target.value};setFields(next)}}><option value="text">Short text</option><option value="textarea">Long text</option><option value="select">Dropdown</option><option value="checkbox">Checkbox</option><option value="date">Date</option></select>{field.type==='select'&&<input placeholder="Options, comma separated" value={(field.options||[]).join(', ')} onChange={e=>{const next=[...fields];next[index]={...field,options:e.target.value.split(',').map(x=>x.trim()).filter(Boolean)};setFields(next)}}/>}<input aria-label="Step name" placeholder="Step (optional)" value={field.step||''} onChange={e=>{const next=[...fields];next[index]={...field,step:e.target.value};setFields(next)}}/><label><input type="checkbox" checked={field.required} onChange={e=>{const next=[...fields];next[index]={...field,required:e.target.checked};setFields(next)}}/> Required</label><button type="button" onClick={()=>setFields(fields.filter((_,i)=>i!==index))}>Remove</button></div>)}
