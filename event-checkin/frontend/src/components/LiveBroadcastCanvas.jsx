...
function DonationTrackerScene({ settings }) {
  ...
  return <div className="flb-donation-scene">
    ...
    <main><section><Kicker>Together, we can reach the goal</Kicker><div className="flb-donation-totals"><article><small>Confirmed gifts</small><h1>{format(confirmed)}</h1><p>received and verified</p></article><article><small>Pledge tracker</small><h1>{format(campaign.pledged_minor || 0)}</h1><p>{campaign.pledge_count || 0} active pledge{campaign.pledge_count === 1 ? '' : 's'} - awaiting fulfilment</p></article></div>{goal > 0 ? <><h2>{format(confirmed)} of {format(goal)} goal confirmed</h2><div className="flb-donation-progress"><i style={{ width: String(pct) + '%' }}/><b>{pct}% confirmed</b></div></> : <div className="flb-donation-mission">Every confirmed gift and pledge moves the mission forward.</div>}</section><aside><img src={'/api/give/' + encodeURIComponent(token) + '/qr.png'} alt="Scan to give or pledge"/><h2>Scan to give or pledge</h2><strong>{settings.donation_url || window.location.origin + '/give/' + token}</strong><small>{channelNames || 'Festio Pay / Cash App / Zelle / Bank transfer / Cash or cheque'}</small></aside></main>
