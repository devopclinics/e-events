...
export default function PublicTicketCheckout({ eventId, tone, onAvailabilityChange }) {
  ...
  return <section className="scroll-mt-6 py-9" id="tickets" aria-labelledby="ticket-checkout-title">
    <div className="rounded-3xl border p-6 shadow-xl sm:p-8" style={{ background: tone.panelStrong, borderColor: tone.border, color: tone.text }}>
      <div className="mb-5"><span className="text-xs font-extrabold uppercase tracking-[.18em]" style={{ color: tone.accent }}>Staging test mode</span><h2 id="ticket-checkout-title" className="mt-1 text-3xl font-extrabold">Buy tickets</h2><p id="checkout-help" style={{ color: tone.muted }}>Secure checkout through {catalog.currency === 'NGN' ? 'Paystack' : 'Stripe'}. No live charge will be made.</p></div>
      <form onSubmit={checkout} className="space-y-5">
        <div className="grid gap-3 sm:grid-cols-2"><input required className="rounded-xl border bg-transparent px-4 py-3" style={{ borderColor: tone.border }} placeholder="First name" value={buyer.firstName} onChange={e => setBuyer({...buyer,firstName:e.target.value})}/><input required className="rounded-xl border bg-transparent px-4 py-3" style={{ borderColor: tone.border }} placeholder="Last name" value={buyer.lastName} onChange={e => setBuyer({...buyer,lastName:e.target.value})}/><input required type="email" className="rounded-xl border bg-transparent px-4 py-3" style={{ borderColor: tone.border }} placeholder="Email" value={buyer.email} onChange={e => setBuyer({...buyer,email:e.target.value})}/><input className="rounded-xl border bg-transparent px-4 py-3" style={{ borderColor: tone.border }} placeholder="Phone (optional)" value={buyer.phone} onChange={e => setBuyer({...buyer,phone:e.target.value})}/></div>
        {catalog.tickets.map(t => t.product_type==='donation' ? <div key={t.id} className="grid gap-3 rounded-2xl border p-4 sm:grid-cols-[1fr_140px]" style={{ borderColor: tone.border }}>
        ...
        {(catalog.checkout_fields||[]).map(field=><label key={field.id} className="block"><span className="mb-1 block text-sm font-bold">{field.label}{field.required?' *':''}</span>{field.type==='textarea'?<textarea required={field.required} className="w-full rounded-xl border bg-transparent px-4 py-3" style={{borderColor:tone.border}} value={customAnswers[field.id]||''} onChange={e=>setCustomAnswers({...customAnswers,[field.id]:e.target.value})}/>:field.type==='select'?<select required={field.required} className="w-full rounded-xl border bg-transparent px-4 py-3" style={{borderColor:tone.border}} value={customAnswers[field.id]||''} onChange={e=>setCustomAnswers({...customAnswers,[field.id]:e.target.value})}><option value="">Choose…</option>{field.options.map(x=><option key={x}>{x}</option>)}</select>:field.type==='checkbox'?<input type="checkbox" required={field.required} checked={!!customAnswers[field.id]} onChange={e=>setCustomAnswers({...customAnswers,[field.id]:e.target.checked})}/>:<input required={field.required} className="w-full rounded-xl border bg-transparent px-4 py-3" style={{borderColor:tone.border}} value={customAnswers[field.id]||''} onChange={e=>setCustomAnswers({...customAnswers,[field.id]:e.target.value})}/>}</label>)}
        {catalog.tax?.enabled&&<p className="rounded-xl border p-3 text-sm" style={{borderColor:tone.border,color:tone.muted}}>{(catalog.tax.bps/100).toFixed(2)}% event tax {catalog.tax.paid_by==='buyer'?'will be added to the customer total at checkout.':'is included in the listed price and paid by the organizer.'}</p>}
        <label className="block max-w-md"><span className="mb-1 block text-sm font-bold">Promo code <small style={{color:tone.muted}}>(optional)</small></span><input className="w-full rounded-xl border bg-transparent px-4 py-3 uppercase" style={{borderColor:tone.border}} maxLength="40" autoComplete="off" placeholder="Enter discount code" value={promoCode} onChange={e=>setPromoCode(e.target.value)}/></label>
