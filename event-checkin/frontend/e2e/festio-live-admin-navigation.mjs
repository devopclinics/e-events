...
export const api = new Proxy({}, { get(_, method) { return async (...args) => {
  ...
  if (method === 'liveBulkUpdateDisplays') {
    const body = args[1];
    return structuredClone(body.display_ids.map(id => {
      const display = test.displays.find(item => item.id === id);
      Object.assign(display, { ...body, settings: { ...display.settings, ...body.settings } });
      delete display.display_ids;
      return display;
    }));
  }
...
async function fixture({ mobile = false, primaryType = 'poll', query = '?tab=Displays' } = {}) {
  ...
  await page.addInitScript(({ primaryType }) => {
    localStorage.setItem('eq.currentEventId', 'event-one')
    const activity = (id, title, type = 'poll') => ({ id, title, type, status: 'live', participant_count: 4, response_count: 3, config: { display_token: `token-${id}` }, questions: [{ id: `q-${id}`, question_type: 'single_choice', prompt: `${title} question`, status: 'active', options: [{ id: 'yes', label: 'Yes' }] }] })
    const display = (id, name, assignedActivityId = 'activity-one') => ({ id, name, display_code: id, access_token: `token-${id}`, assigned_activity_id: assignedActivityId, assigned_session_id: null, scene: 'join', settings: {}, status: 'active', connected: true, connected_count: 2, connection_limit: 2, connection_status_available: true, devices: [{ client_id: 'device-one', last_seen_at: new Date().toISOString() }, { client_id: 'device-two', last_seen_at: new Date().toISOString() }] })
...
try {
  ...
  {
    for (const primaryType of ['survey', 'feedback']) {
      const { page, context } = await fixture({ primaryType })
      const main = card(page)
      await expect(main.getByRole('button', { name: 'Download report', exact: true })).toBeVisible()
      await expect(main.getByText(`Download the complete ${primaryType} report without changing this display.`, { exact: true })).toBeVisible()
      await main.getByRole('button', { name: 'Download report', exact: true }).click()
      const reportCalls = await page.evaluate(() => window.__liveTest.calls.filter(call => call.method === 'liveDownloadSurveyReport'))
      assert.equal(reportCalls.length, 1)
      assert.deepEqual(reportCalls[0].args, ['event-one', 'activity-one', `Opening ${primaryType}`])
      assert.equal(await page.evaluate(() => window.__liveTest.calls.filter(call => call.method === 'liveUpdateDisplay').length), 0, 'Downloading a report must not update the display')
      await context.close()
    }
    console.log('PASS: survey and feedback display report downloads leave the display assignment unchanged')
  }
  {
    const { page, context } = await fixture({ query: '?tab=Control%20Room' })
    await page.locator('.fl-control-activity').filter({ hasText: 'Opening poll' }).click()
    await expect(page.getByText('Opening poll question', { exact: true })).toBeVisible()
    const targets = page.getByRole('checkbox')
    await expect(targets).toHaveCount(2)
    await expect(targets.nth(0)).toBeChecked()
    await expect(targets.nth(1)).not.toBeChecked()
    await page.locator('.fl-control-activity').filter({ hasText: 'Closing poll' }).click()
    await expect(page.getByText('Closing poll question', { exact: true })).toBeVisible()
    await expect(targets.nth(0)).not.toBeChecked()
    await expect(targets.nth(1)).toBeChecked()
    await page.locator('.fl-control-activity').filter({ hasText: 'Opening poll' }).click()
    await expect(page.getByText('Opening poll question', { exact: true })).toBeVisible()
    await expect(targets.nth(0)).toBeChecked()
    await expect(targets.nth(1)).not.toBeChecked()
    await targets.nth(1).check()
    await page.getByRole('combobox', { name: 'Scene on selected channels', exact: true }).selectOption('results')
    await page.getByRole('button', { name: 'Push Current result to 2 selected', exact: true }).first().click()
    await expect(page.getByRole('status')).toContainText('Opening poll is now on 2 selected channels.')
    const bulkCalls = await page.evaluate(() => window.__liveTest.calls.filter(call => call.method === 'liveBulkUpdateDisplays'))
    assert.equal(bulkCalls.length, 1)
    assert.deepEqual(bulkCalls[0].args[1].display_ids, ['display-one', 'display-two'])
    assert.equal(bulkCalls[0].args[1].assigned_activity_id, 'activity-one')
    assert.equal(bulkCalls[0].args[1].scene, 'results')
    assert.equal(bulkCalls[0].args[1].settings.follow_activity, false, 'A named scene must not be overridden by guided follow mode')
    assert.equal(await page.evaluate(() => window.__liveTest.calls.filter(call => call.method === 'liveUpdateDisplay').length), 0, 'Control Room uses one multi-channel update')
    await page.getByRole('button', { name: 'Clear content', exact: true }).click()
    const clearCall = await page.evaluate(() => window.__liveTest.calls.filter(call => call.method === 'liveBulkUpdateDisplays').at(-1))
    assert.equal(clearCall.args[1].assigned_activity_id, null)
    assert.equal(clearCall.args[1].scene, 'welcome')
    await context.close()
    console.log('PASS: unified control room selects multiple channels and clears them together')
  }
  {
