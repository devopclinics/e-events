...
def render_site(content: dict, family: str, *, preview: bool = False) -> str:
    ...
    background_hero = family in {"modern-professional", "storytelling", "immersive", "elegant-countdown"}
    image_style = f"background-image:linear-gradient(90deg,rgba(0,0,0,.72),rgba(0,0,0,.12)),url('{hero}')" if hero and background_hero else ""
    ...
    section_orders = {
        ...
        "clean-elegant": (glance, registration, audience, programme, speakers_section, features, venue, connect, faq_section, contact),
        "split-visual": (glance, audience, programme, speakers_section, features, venue, registration, connect, faq_section, contact),
        "storytelling": (glance, features, audience, programme, speakers_section, venue, connect, registration, faq_section, contact),
        "immersive": (glance, features, programme, audience, speakers_section, venue, connect, registration, faq_section, contact),
    }
    main_sections = "".join(section_orders.get(family, (glance, registration, programme, audience, speakers_section, features, venue, connect, faq_section, contact)))
...
.hero{{min-height:65vh;padding:7vw max(6vw,30px);display:grid;align-content:center;background-size:cover;background-position:center;position:relative;overflow:hidden}}
.hero-inner{{width:min(1180px,100%);margin:auto;display:grid;grid-template-columns:minmax(0,1.05fr) minmax(320px,.95fr);gap:clamp(2rem,6vw,6rem);align-items:center}}.hero-copy{{position:relative;z-index:2}}.hero-visual{{min-height:480px;position:relative;overflow:hidden;border-radius:32px;box-shadow:var(--shadow)}}.hero-visual img{{width:100%;height:100%;position:absolute;inset:0;object-fit:cover}}.hero-placeholder{{display:grid;place-items:center;background:linear-gradient(135deg,var(--primary),var(--accent));color:white;font-size:8rem}}.eyebrow{{text-transform:uppercase;letter-spacing:.18em;font-weight:800;color:var(--accent);font-size:.8rem}}h1{{font:700 clamp(3rem,8vw,7rem)/.94 Georgia,serif;margin:.25em 0}}h2{{font:700 clamp(1.8rem,3.4vw,2.8rem)/1.05 Georgia,serif;margin:.2em 0 .5em}}.lead{{font-size:clamp(1.05rem,2vw,1.35rem);max-width:680px}}.meta{{font-weight:700;margin:1.5rem 0}}.countdown-chip{{display:inline-flex;align-items:center;font-size:.8rem;font-weight:800;padding:.35rem .8rem;border-radius:999px;background:var(--accent);color:#17110d}}
...
.modern-professional .hero{{background-color:#092d50;color:white;border-radius:0 0 42px 42px}}.modern-professional .hero-inner,.storytelling .hero-inner,.immersive .hero-inner,.elegant-countdown .hero-inner{{display:block;margin:0;max-width:780px}}
.clean-elegant{{--paper:#f8f5ee;--ink:#132921;--card-radius:8px}}.clean-elegant nav{{background:var(--paper)}}.clean-elegant .hero{{min-height:76vh;padding-block:5vw}}.clean-elegant .hero-visual{{border-radius:48% 48% 42% 58%;box-shadow:none;min-height:560px}}.clean-elegant .hero-visual:after{{content:'';position:absolute;width:42%;height:42%;right:-5%;bottom:-6%;border-radius:50%;background:color-mix(in srgb,var(--accent) 24%,white);border:10px solid var(--paper)}}.clean-elegant h1{{font-size:clamp(3.6rem,7vw,6.5rem)}}.clean-elegant main>section{{border-bottom:1px solid var(--border);padding-bottom:4rem}}
...
.bold-dynamic{{--paper:#071b3f;--ink:#f5f8ff;--card-radius:6px}}.bold-dynamic nav,.bold-dynamic article,.bold-dynamic .fact,.bold-dynamic .day-tab,.bold-dynamic .track-pill{{background:#102752;color:#fff;border-color:#ffffff22}}.bold-dynamic .hero{{color:white;background:linear-gradient(120deg,#071b3f 0 58%,#102b5c 58%);clip-path:polygon(0 0,100% 0,100% 88%,74% 100%,0 93%)}}.bold-dynamic .hero-visual{{border-radius:0;clip-path:polygon(22% 0,100% 0,100% 100%,0 86%);box-shadow:none}}.bold-dynamic h1{{font-family:Inter,sans-serif;text-transform:uppercase}}.bold-dynamic main{{color:#fff}}
.card-friendly{{--paper:#edf7f7;--card-radius:24px}}.card-friendly .hero{{min-height:56vh;margin:2rem auto;max-width:var(--site-max-width);border-radius:28px;background-color:#fff;box-shadow:var(--shadow);padding:4rem}}.card-friendly .hero-visual{{min-height:380px;border-radius:22px}}.card-friendly main>section{{background:#fff;padding:2.2rem;border-radius:24px;box-shadow:0 8px 25px #174b4210}}
.conference-programme .hero{{min-height:34vh;background-color:#f0eee9;border-left:16px solid var(--accent);padding-block:3rem}}.conference-programme .hero-inner{{grid-template-columns:1fr 280px}}.conference-programme .hero-visual{{min-height:240px;border-radius:8px}}.conference-programme h1,.programme-showcase h1{{font-family:Inter,system-ui,sans-serif;letter-spacing:-.06em}}.conference-programme #programme{{margin-top:-2rem;background:white;padding:2rem;border-radius:18px;box-shadow:var(--shadow)}}
.split-visual .hero{{min-height:72vh;padding-block:0}}.split-visual .hero-inner{{width:100%;max-width:none;grid-template-columns:45% 55%;gap:0}}.split-visual .hero-copy{{padding:6vw}}.split-visual .hero-visual{{min-height:72vh;border-radius:0;clip-path:polygon(18% 0,100% 0,100% 100%,0 100%);box-shadow:none}}
...
.programme-showcase .hero{{min-height:42vh;background:linear-gradient(135deg,#fff,var(--paper))}}.programme-showcase .hero-visual{{min-height:290px;border-radius:12px}}.programme-showcase #tracks{{margin-top:-2rem}}.programme-showcase ul.tracks{{grid-template-columns:repeat(auto-fit,minmax(150px,1fr))}}.programme-showcase .track{{min-height:210px;color:#fff;background:var(--track-color);display:flex;flex-direction:column;justify-content:end}}.programme-showcase .track-jump{{color:#fff}}
...
@media(max-width:900px){{.hero-inner,.split-visual .hero-inner,.conference-programme .hero-inner{{grid-template-columns:1fr}}.hero-visual,.clean-elegant .hero-visual,.split-visual .hero-visual{{min-height:380px;border-radius:20px;clip-path:none}}.split-visual .hero-copy{{padding:4rem max(5vw,24px)}}.split-visual .hero{{padding-bottom:0}}}}
