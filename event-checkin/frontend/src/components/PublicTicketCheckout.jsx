  ...
  return <section className="scroll-mt-6 py-9" id="tickets" aria-labelledby="ticket-checkout-title">
    <div className="rounded-3xl border p-6 shadow-xl sm:p-8" style={{ background: tone.panelStrong, borderColor: tone.border, color: tone.text }}>
      ...
      <form onSubmit={checkout} className="space-y-5">
        ...
        {hasSellableTickets && (catalog.checkout_fields||[]).map(field=><label key={field.id} className="block"><span className="mb-1 block text-sm font-bold">{field.label}{field.required?' *':''}</span>{field.type==='textarea'?<textarea required={field.required} className="w-full rounded-xl border bg-transparent px-4 py-3" style={{borderColor:tone.border}} value={customAnswers[field.id]||''} onChange={e=>setCustomAnswers({...customAnswers,[field.id]:e.target.value})}/>:field.type==='select'?<select required={field.required} className="w-full rounded-xl border bg-transparent px-4 py-3" style={{borderColor:tone.border}} value={customAnswers[field.id]||''} onChange={e=>setCustomAnswers({...customAnswers,[field.id]:e.target.value})}><option value="">Choose…</option>{field.options.map(x=><option key={x}>{x}</option>)}</select>:field.type==='checkbox'?<input type="checkbox" required={field.required} checked={!!customAnswers[field.id]} onChange={e=>setCustomAnswers({...customAnswers,[field.id]:e.target.checked})}/>:<input required={field.required} className="w-full rounded-xl border bg-transparent px-4 py-3" style={{borderColor:tone.border}} value={customAnswers[field.id]||''} onChange={e=>setCustomAnswers({...customAnswers,[field.id]:e.target.value})}/>}</label>)}
