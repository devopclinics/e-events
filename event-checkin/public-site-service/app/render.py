...
<header class="hero" style="{image_style}"><div class="hero-inner"><div class="eyebrow">{eyebrow}</div><h1>{headline}</h1><p class="lead">{summary}</p><div class="meta">{meta}{countdown_markup(content)}</div><div class="actions">{action(content.get("primary_action"))}{action(content.get("secondary_action"), "button outline")}</div></div></header>
