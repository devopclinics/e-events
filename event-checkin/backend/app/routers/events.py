...
from ..schemas import ActiveToggle, EventCodeUpdate
...
async def set_event_code(
    event_id: str,
    body: EventCodeUpdate,
    db: AsyncSession = Depends(get_db),
    _: User = Depends(require_event_admin),
):
    """Sets a custom short code (e.g. "iedpu26"), shared by self check-in and
    Giving Hub links, in place of the random one generated by default."""
    event = await db.get(Event, event_id)
    if not event:
        raise HTTPException(404, "Event not found")
    code = body.event_code.strip().lower()
    if not re.fullmatch(r"[a-z0-9-]+", code):
        raise HTTPException(422, "Event code may only contain letters, numbers, and hyphens")
    existing = await db.scalar(select(Event.id).where(func.lower(Event.event_code) == code, Event.id != event_id))
    if existing:
        raise HTTPException(409, "That code is already in use by another event")
