...
const emptyProduct = { name: '', description: '', price: '', currency: 'USD', capacity: '', min_per_order: 1, max_per_order: 10, access_ticket_type_id: null, active: true, product_type: 'ticket', allow_custom_amount: false }
    ...
    {!eventId ? <div className="rr-panel">Select an event first.</div> : config && !config.service_enabled ?
      ...
      {config?.config?.enabled && <>
        ...
        <section className="rr-panel tk-section"><div className="tk-section-head"><div><span className="tk-step">03 · Ticket types</span><h2>Add tickets</h2><p>Add General Admission, VIP, Early Bird, tables, donation tiers, or any other line item. Submit this form again for every type.</p></div><span className="tk-count">{products.length} {products.length===1?'type':'types'}</span></div><form className="tk-form tk-ticket-form" onSubmit={createProduct}>
          <label><span>Type</span><select value={draft.product_type} onChange={e=>setDraft({...draft, product_type:e.target.value, allow_custom_amount: e.target.value==='donation'?draft.allow_custom_amount:false})}><option value="ticket">Ticket (grants admission)</option><option value="donation">Donation / sponsorship (payment only, no pass)</option></select></label>
        ...
        <section className="rr-panel tk-section"><div className="tk-section-head"><div><span className="tk-step">04 · Catalog</span><h2>Your tickets</h2></div></div>{products.length ? <div className="tk-list">{products.map(p => <div key={p.id}><span className="tk-ticket-icon">{p.name.slice(0,1).toUpperCase()}</span><span className="tk-ticket-name"><strong>{p.name}{p.product_type==='donation' && ' 🎁'}{!p.active && ' (hidden)'}</strong><small>{p.product_type==='donation'?(p.allow_custom_amount?`${money(p.price,p.currency)}+ pledged`:'fixed tier'):`${p.sold} sold`} · {p.capacity-p.sold} remaining</small></span><strong>{money(p.price, p.currency)}{p.product_type==='donation'&&p.allow_custom_amount?'+':''}</strong><button disabled={busy} onClick={async()=>{if(!window.confirm(`Hide ${p.name} from sale?`))return;setBusy(true);try{await api.deleteTicketingProduct(eventId,p.id);await load()}catch(e){setError(e.message)}finally{setBusy(false)}}}>Hide</button></div>)}</div> : <div className="tk-empty"><span>🎟</span><strong>No ticket types yet</strong><p>Create your first ticket above. You can add as many types as the event needs.</p></div>}</section>
