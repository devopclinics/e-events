...
const defaultNavigation = [
  ...
  { id: 'festiome', label: 'FestioMe', destination_type: 'festiome', url: '', enabled: true },
...
function mergeNavigation(items, sources) {
  ...
  return merged.map(item => {
    ...
    return url !== undefined ? { ...item, url } : item
  })
}
...
export default function EventWebsitePage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false
    ;(async () => {
      const [existing, events, connectionResult] = await Promise.allSettled([api.website(eventId), api.listEvents(), api.websiteConnections(eventId)])
  ...
  async function save(){setBusy(true);try{const payload={...site,content:{...content,navigation:(content.navigation||[]).map(item=>({...item,url:navUrl(item),enabled:item.enabled}))}};const value=await api.saveWebsite(eventId,payload);setSite(value);setStatus('Draft saved');return value}catch(e){setStatus(e.message);throw e}finally{setBusy(false)}}
  ...
  }),[activeTab,content,releases,site.slug,site.template_family,sources,catalog])
