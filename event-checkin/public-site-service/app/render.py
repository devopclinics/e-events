...
main{{max-width:1180px;margin:auto;padding:5rem max(5vw,24px)}}main>section{{margin-bottom:5rem}}.schedule{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem}}article{{background:white;border:1px solid #e8e1d5;padding:1.4rem;border-radius:18px}}article time{{color:var(--accent);font-weight:800}}article h3{{margin:.45rem 0}}ul.tracks{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:1rem;padding:0;list-style:none}}
.day-links{{display:flex;gap:.5rem;flex-wrap:wrap;margin:0 0 1.4rem}}.day-links a{{border:1px solid #d9d1c5;background:#fff;padding:.5rem .9rem;border-radius:999px;font-weight:700;text-decoration:none;font-size:.85rem}}.schedule-day{{scroll-margin-top:90px;margin:2rem 0}}.schedule-day>h3{{font:700 1.4rem Georgia,serif;border-bottom:2px solid var(--accent);padding-bottom:.5rem}}.schedule-day>h3 small{{display:block;font:600 .7rem Inter;margin-top:.3rem;color:#6b746f}}.sessions{{display:grid;gap:.7rem}}.session-action{{font-weight:800;color:var(--accent);display:inline-block;margin-top:.5rem}}
