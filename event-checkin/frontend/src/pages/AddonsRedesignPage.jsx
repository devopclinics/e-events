...
function RealLogisticsContent({ eventId, notify }) {
  const blank = { name: '', phase: 'pre', collect_size: true, auto_add: true, notes: '', vendor_name: '', vendor_email: '', vendor_phone: '' }
  ...
  function openEditor(item = null) {
    ...
    setForm(item ? {
      ...
      auto_add: !!item.auto_add, notes: item.notes || '', vendor_name: item.vendor_name || '',
  ...
  async function save() {
    ...
    setBusy(true)
    try {
      const payload = { ...form, name: form.name.trim(), notes: form.notes || null, vendor_name: form.vendor_name || null, vendor_email: form.vendor_email || null, vendor_phone: form.vendor_phone || null }
...
function RealRegistryContent({ eventId, notify }) {
  const blank = { kind: 'item', title: '', description: '', external_url: '', amount: '', currency: 'USD', quantity_wanted: 1, payment_instructions: '' }
  ...
  return <>
      ...
      <div className="rr-panel"><div className="rd-panel-head"><h3>Gift list</h3><p>{items.length} item{items.length === 1 ? '' : 's'}</p></div><div className="rd-panel-body">
        {items.length === 0 ? <EmptyState icon="gift" title="No gifts yet" message="Add an item, cash fund, or external link." /> : items.map((item) => (
          <div className="ad-registry-item" key={item.id}>
            <div className="ad-registry-item-top"><span><span className={`ad-kind-badge ${item.kind}`}>{item.kind}</span> {item.title}</span><span>{item.claim_count || 0} claims</span></div>
    ...
    {editing && <Modal title={editing === 'new' ? 'Add registry item' : `Edit ${editing.title}`} onClose={() => setEditing(null)} width={500}>
      ...
      {form.kind === 'link' && <><label className="rd-field-label">External URL</label><input className="rd-field" type="url" value={form.external_url} onChange={(e) => setForm((v) => ({ ...v, external_url: e.target.value }))} /></>}
