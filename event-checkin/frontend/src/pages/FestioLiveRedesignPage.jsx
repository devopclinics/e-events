...
function DisplayCard({ display, draft, onDraftChange, onDraftApplied, eventId, activities, programSessions, busy, onUpdate, onDelete, onDisconnect, onPresentResults, onRehearsal }) {
  ...
  const assignedReportActivity = activities.find((activity) => activity.id === display.assigned_activity_id)
  const canDownloadReport = ['survey', 'feedback'].includes(assignedReportActivity?.type)
  ...
  // The report request contains no display ID: it renders server-side and
  // leaves the screen's connection, scene, and operator draft untouched.
  async function downloadAssignedReport() {
    if (!canDownloadReport || !assignedReportActivity) return
    setReportDownloading(true)
    setReportError('')
    try {
      await api.liveDownloadSurveyReport(eventId, assignedReportActivity.id, assignedReportActivity.title)
    } catch (error) {
      setReportError(error?.message || 'The report could not be downloaded. Please try again.')
    } finally {
  ...
  return <article className="fl-display-card">
    ...
    <div className="fl-results-quickbar" aria-label="Results and rehearsal controls">
      ...
      {canDownloadReport && <>
        <button type="button" className="rr-btn secondary" disabled={reportDownloading} onClick={downloadAssignedReport}>{reportDownloading ? 'Preparing report…' : 'Download report'}</button>
        <small role={reportError ? 'alert' : undefined} style={reportError ? { color: '#a3271e' } : undefined}>{reportError ? 'Report download failed: ' + reportError : 'Download the complete ' + assignedReportActivity.type + ' report without changing this display.'}</small>
      </>}
...
function UnifiedControlRoom({ activities, displays, selected, loadingActivityId, busy, onSelectActivity, onOpenControls, onAdvance, advanceLabel, onPush, onClear, onManageDisplay, onOpenPresenter, onNewActivity }) {
  ...
  const previousActivityId = useRef(selected?.id)

  // Changing activities must never silently retain the prior activity's
  // targets. For the newly selected activity, restore only its own channels;
  // when it has none, require the operator to choose them explicitly.
  // Ordinary display-list refreshes retain the current deliberate selection.
  useEffect(() => {
    const activityChanged = previousActivityId.current !== selected?.id
    previousActivityId.current = selected?.id
    setTargetIds((current) => {
      const valid = current.filter((id) => allDisplays.some((display) => display.id === id))
      if (activityChanged && selected) return allDisplays.filter((display) => display.assigned_activity_id === selected.id).map((display) => display.id)
      if (valid.length) return valid
      if (selected) return allDisplays.filter((display) => display.assigned_activity_id === selected.id).map((display) => display.id)
      return allDisplays.slice(0, 1).map((display) => display.id)
    })
  }, [displays, selected?.id])

  useEffect(() => {
    if (selected && !compatibleScenes(selected.type).includes(scene)) setScene('join')
  }, [selected?.type]) // eslint-disable-line react-hooks/exhaustive-deps

  const selectedDisplays = allDisplays.filter((display) => targetIds.includes(display.id))
  const currentQuestion = selected?.questions?.find((question) => question.id === selected?.config?.current_question_id)
  ...
  function toggleTarget(displayId) {
    setReceipt('')
    setTargetIds((current) => current.includes(displayId) ? current.filter((id) => id !== displayId) : [...current, displayId])
  }

  async function pushSelected() {
    if (!selected || !targetIds.length || busy || loadingActivityId) return
    const updated = await onPush(targetIds, scene)
    if (updated) setReceipt(`${selected.title} is now on ${updated.length} selected channel${updated.length === 1 ? '' : 's'}.`)
  }

  async function clearSelected() {
    if (!targetIds.length || busy) return
    const updated = await onClear(targetIds)
    if (updated) setReceipt(`Welcome screen sent to ${updated.length} selected channel${updated.length === 1 ? '' : 's'}. Devices remain connected.`)
  }
  ...
  return <section className="fl-control-room" aria-label="Unified Control Room">
    ...
    <div className="fl-control-room-shell">
      <aside className="fl-control-rail" aria-label="Activities and shows">
        <div className="fl-control-rail-head"><span>Activities &amp; shows</span><small>Stays open while you operate</small></div>
        ...
        <div className="fl-control-rail-foot"><span>No content</span><p>Welcome / agenda can be sent at any time, including while no activity is selected.</p><button className="rr-btn secondary" disabled={busy || !targetIds.length} onClick={clearSelected}>Show welcome on selected</button></div>
      </aside>

      <main className="fl-control-main">
        {loadingActivityId && <div className="fl-loading" role="status">Loading activity controls…</div>}
        {!loadingActivityId && !selected && <div className="fl-control-empty fl-control-main-empty"><span>Choose an activity</span><h3>Start from the activity rail.</h3><p>Its live controls will remain here while you select the channels that should show it.</p></div>}
        {!loadingActivityId && selected && <>
          <section className="fl-control-stage">
            <div><span>{selected.status === 'live' ? '● Live now' : selected.status}</span><h3>{currentQuestion?.prompt || selected.title}</h3><p>{selected.title}{currentQuestion ? ` · ${currentQuestion.question_type.replaceAll('_', ' ')}` : ''}</p></div>
            <div className="fl-control-stat-grid"><div><strong>{selected.participant_count || 0}</strong><small>participants</small></div><div><strong>{selected.response_count || 0}</strong><small>responses</small></div><div><strong>{selectedDisplays.length}</strong><small>channels targeted</small></div></div>
          </section>
          <div className="fl-control-actions" aria-label="Activity controls">
          ...
          <p className="fl-control-note">Changing or clearing a channel keeps its connected TVs online. A channel that is not selected is not changed.</p>
          {receipt && <p className="fl-control-receipt" role="status" aria-live="polite">{receipt}</p>}
        </>}
      </main>

      <aside className="fl-control-targets" aria-label="Channels">
        <header><div><span>Channels</span><small>Each channel can mirror to several physical screens.</small></div><b>{targetIds.length} selected</b></header>
        <div className="fl-control-target-list">
          {allDisplays.map((display) => {
            const targeted = targetIds.includes(display.id)
            const assignedActivity = allActivities.find((activity) => activity.id === display.assigned_activity_id)
            return <article key={display.id} className={`fl-control-target${targeted ? ' selected' : ''}`}>
              <label className="fl-control-target-head"><input type="checkbox" checked={targeted} disabled={busy} onChange={() => toggleTarget(display.id)} /><span><strong>{display.name}</strong><small>{assignedActivity ? `Showing ${assignedActivity.title}` : 'Welcome / no activity'}</small></span><b>{display.connected_count ?? (display.connected ? 1 : 0)} screen{(display.connected_count ?? (display.connected ? 1 : 0)) === 1 ? '' : 's'}</b></label>
              <div className="fl-control-target-preview">{display.scene === 'welcome' ? '✦ welcome scene' : `▥ ${DISPLAY_SCENES.find(([key]) => key === display.scene)?.[1] || display.scene}`}</div>
              <footer><span className={`fl-screen-status ${display.connected ? 'connected' : ''}`}>{displayConnectionLabel(display)}</span><button type="button" className="rr-link-btn" onClick={() => onManageDisplay(display.id)}>Manage devices</button></footer>
            </article>
          })}
          {!allDisplays.length && <div className="fl-control-empty"><span>No channels</span><p>Add a channel in Broadcast studio, then connect its link on the TV or projector.</p><button className="rr-btn primary" onClick={() => onManageDisplay(null)}>Add channel</button></div>}
        </div>
        {allDisplays.length > 0 && <footer className="fl-control-targets-foot"><p><b>Observer previews</b> are read-only and never claim a device slot.</p><button className="rr-btn primary" disabled={busy || !selected || !targetIds.length || !!loadingActivityId} onClick={pushSelected}>Push to {targetIds.length} selected channel{targetIds.length === 1 ? '' : 's'}</button></footer>}
      </aside>
    </div>
  </section>
}
...
function FestioLiveEventPage({ eventId }) {
  ...
  async function pushControlRoomActivity(displayIds, scene) {
    if (!selected || loadingActivityId || !displayIds.length) return null
    // Join is the only Control Room scene that intentionally follows the
    // guided show. A named scene must stay exactly as selected by the operator.
    const followActivity = scene === 'join'
    return bulkUpdateDisplays(displayIds, {
      assigned_activity_id: selected.id,
      assigned_session_id: selected.session_id || null,
      scene,
      settings: { control_mode: followActivity ? 'guided' : 'manual', follow_activity: followActivity, auto_follow_program: false, rehearsal_mode: false, results_frozen: false, results_snapshot: null },
    })
  }

  async function clearControlRoomChannels(displayIds) {
    if (!displayIds.length) return null
    return bulkUpdateDisplays(displayIds, {
      assigned_activity_id: null,
  ...
  async function bulkUpdateDisplays(displayIds, patch) {
    if (!displayIds.length) return null
    displayRevision.current += 1; displayMutations.current += 1
    setBusy(true); setError('')
    try {
      const updated = await api.liveBulkUpdateDisplays(eventId, { display_ids: displayIds, ...patch })
      const updatedById = new Map(updated.map((display) => [display.id, display]))
      setDisplays((current) => (current || []).map((display) => updatedById.get(display.id) || display))
      return updated
    } catch (e) { setError(e.message); return null } finally { displayRevision.current += 1; displayMutations.current -= 1; setBusy(false) }
  }
  ...
  return (
    <RedesignShell topActive="live" withEventSidebar eventActive="live">
      ...
      {event?.experience_enabled && ['Control Room', 'Activities', 'Live Control', 'Displays', 'Responses', 'Analytics'].includes(tab) && <div className="fl-program-filter">
      ...
      {tab === 'Control Room' && <UnifiedControlRoom
        activities={activities}
        displays={displays}
        selected={selected}
        ...
        onOpenControls={() => setTab('Activities')}
        onAdvance={selected?.config?.show_mode === 'guided' && selected?.config?.show_phase !== 'complete' ? advanceGuidedShow : startGuidedShow}
        advanceLabel={selected ? guidedActionLabel(selected) : 'Start guided show'}
